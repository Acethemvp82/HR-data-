...
if run:
    ...
    with tabs[2]:
        ...
        st.download_button("Download full CSV",rankings.to_csv(index=False).encode(),
                           file_name=f"mlb_hr_v3_{selected}.csv",mime="text/csv")
    st.markdown("<div class='note'><b>V3:</b> Recent hitter contact drives the board. Pitch-type matchup and "
...
with st.expander("ℹ️ What changed in V3"):
    st.markdown("""
...
- **3/7 minimum:** fewer than 3 contact confirmations cannot qualify as GOOD (80+).
- **ELITE protection:** a hitter needs at least 4/7 L10 contact confirmations before the score can reach 90+.
